...
class Candidate:
    ...
    Attributes:
        name: Name of the candidate. It should be unique enough for the user to be
            able to distinguish between candidates based on the name alone.
    """
...
def get_background_files(trainfile):
    ...
    file as a list of strings. Paths are from the current directory.
    """
...
def get_queries(filename):
    ...
    if filename.split('.')[-1] == 'train':
        return parse_train(filename)
    elif filename.split('.')[-1] == 'fax':
        return parse_fax(filename)
    else:
